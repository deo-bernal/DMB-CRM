...
namespace Dmb.Crm.Service.Implementation.Email;
...
public class EmailService : IActivationEmailSender, IPasswordResetEmailSender, IExternalLoginEmailSender
{
    ...
    {
        var html = $@"<h2>Activate your DMB CRM account</h2>
...
<p>If you did not register, ignore this email.</p>";
    ...
    {
        var html = $@"<h2>Reset your DMB CRM password</h2>
...
<p><a href=""{WebUtility.HtmlEncode(resetLink)}"">Reset password</a></p>";
    ...
    {
        var html = $@"<h2>Your DMB CRM sign-in code</h2>
...
<p>It expires in 15 minutes. If you did not try to sign in, ignore this email.</p>";
        return SendAsync(toEmail, "Your DMB CRM sign-in code", html, cancellationToken);
    }
    ...
    private async Task SendAsync(string toEmail, string subject, string html, CancellationToken cancellationToken)
    {
        ...
        {
            From = new MailAddress(_configuration["Smtp:From"] ?? "noreply@dmbwebsolutions.com", "DMB CRM"),
            Subject = subject,
            Body = html,
            IsBodyHtml = true
        };

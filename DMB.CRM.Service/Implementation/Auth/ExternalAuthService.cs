...
public class ExternalAuthService : IExternalAuthService
{
    ...
    public ExternalAuthStartResult Start(string provider, string? redirect, string callbackUrl)
    {
        ...
        var state = CreateState(redirect);
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        if (!string.IsNullOrWhiteSpace(profile.Email))
        {
            ...
            if (user is not null)
            {
                ...
                await EnsureLocationMembershipAsync(user.Id, user.AgencyId, cancellationToken);
    ...
    private async Task<CrmUser> CreateSocialUserAsync(OAuthProfile profile, string email, CancellationToken cancellationToken)
    {
        var agency = await _db.Agencies.FirstOrDefaultAsync(a => a.Slug == "dmb", cancellationToken)
            ?? throw new InvalidOperationException("Default CRM agency was not found.");
    ...
    private async Task EnsureLocationMembershipAsync(Guid userId, Guid agencyId, CancellationToken cancellationToken)
    {
    ...
    private static string CleanSecret(string? value) => (value ?? "").Trim().Trim('"').Trim('\'');
    ...
    private string CreateState(string? redirect)
    {
        ...
        return "crm." + ToBase64Url(payloadBytes) + "." + ToBase64Url(signature);
    }
    ...
    private OAuthStatePayload? ParseState(string? state)
    {
        ...
        if (state.StartsWith("crm.", StringComparison.Ordinal))
        {
            state = state[4..];
        }

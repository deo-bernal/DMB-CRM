            ...
            return (
              <div className="user-row" key={user.userId}>
                ...
                <div className="user-actions">
                  <button className="secondary" type="button" disabled={busyUserId === user.userId} onClick={() => openEdit(user)}>Edit</button>
                  <button className="danger" type="button" disabled={!canDelete || busyUserId === user.userId} onClick={() => { setError(""); setDeletingUser(user); }}>Delete</button>
                </div>
              </div>
            );
          })
        ...
        <div className="dialog-backdrop" onClick={() => { if (!busyUserId) { setEditingUser(null); setDraft(null); } }}>
          <form className="card dialog-card" onClick={(e) => e.stopPropagation()} onSubmit={(e) => void handleSave(e)}>
            ...
            <label className="field" style={{ display: "flex", gap: "0.5rem", alignItems: "center" }}>
              <input type="checkbox" checked={draft.activated} onChange={(e) => setDraft({ ...draft, activated: e.target.checked })} style={{ width: "auto" }} />
              Activated
            </label>
            <div className="user-actions">
              <button className="secondary" type="button" disabled={Boolean(busyUserId)} onClick={() => { setEditingUser(null); setDraft(null); }}>Cancel</button>
      ...
      {deletingUser ? (
        <div className="dialog-backdrop" onClick={() => { if (!busyUserId) setDeletingUser(null); }}>
          <div className="card dialog-card" onClick={(e) => e.stopPropagation()}>
            <h2>Delete user</h2>
            <p>Permanently delete {displayName(deletingUser)} ({deletingUser.email})? If they still have CRM records, the account will be deactivated instead.</p>
            <div className="user-actions">
              <button className="secondary" type="button" disabled={Boolean(busyUserId)} onClick={() => setDeletingUser(null)}>Cancel</button>
              <button className="danger" type="button" disabled={Boolean(busyUserId)} onClick={() => void handleDelete()}>{busyUserId ? "Deleting…" : "Delete user"}</button>
            </div>
          </div>
        </div>
      ) : null}
